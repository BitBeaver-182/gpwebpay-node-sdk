...
describe('VRCode', () => {
  ...
  it('should throw InvalidArgumentException for a long VRCode', () => {
    ...
    expect(() => new VRCode(invalidString)).toThrowError(InvalidArgumentException);
    expect(() => new VRCode(invalidString)).toThrowError(`VRCODE max. length is 48! "${invalidString.length}" given.`);
  });
});