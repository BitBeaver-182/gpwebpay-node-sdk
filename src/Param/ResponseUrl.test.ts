...
describe('ResponseUrl', () => {
  ...
  it('should throw InvalidArgumentException for an invalid URL', () => {
    expect(() => new ResponseUrl('tohlenenniurl')).toThrowError(InvalidArgumentException);
    expect(() => new ResponseUrl('tohlenenniurl')).toThrowError('URL is Invalid.');
  });

  it('should throw InvalidArgumentException for a long URL', () => {
    const longUrl = 'http://' + faker.string.alpha({ length: 300 });
    expect(() => new ResponseUrl(longUrl)).toThrowError(InvalidArgumentException);
    expect(() => new ResponseUrl(longUrl)).toThrowError(`URL max. length is 300! "${longUrl.length}" given.`);
  });
});