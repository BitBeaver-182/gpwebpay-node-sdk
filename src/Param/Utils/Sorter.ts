...
export class Sorter {
	 ...
	 */
	public static sortRequestParams<T>(
	 ...
	 * @returns Record<string, T>
	 */
	public static sortResponseParams<T>(
		params: Record<string, T>,
	): Record<string, T> {
		const order: Record<string, number> = {};
	...
	private static sort<T>(
		...
		const sortedKeys = Object.keys(params).sort((a, b) => {
			const indexA =
				order[a] !== undefined ? order[a] : Number.POSITIVE_INFINITY;
			const indexB =
				order[b] !== undefined ? order[b] : Number.POSITIVE_INFINITY;
