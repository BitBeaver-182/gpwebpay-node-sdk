...
describe('OrderNumber', () => {
  ...
  it('should throw InvalidArgumentException for an invalid scalar value', () => {
    ...
    expect(() => new OrderNumber(invalidValue)).toThrowError(InvalidArgumentException);
    expect(() => new OrderNumber(invalidValue))
      .toThrowError(`ORDERNUMBER must be integer "${invalidValue}" given.`);
  });
});