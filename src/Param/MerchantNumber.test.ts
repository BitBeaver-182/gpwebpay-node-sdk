...
describe('MerchantNumber', () => {
  ...
  it('should throw InvalidArgumentException for a long merchant number', () => {
    ...
    expect(() => new MerchantNumber(invalidString)).toThrowError(InvalidArgumentException);
    expect(() => new MerchantNumber(invalidString)).toThrowError(`MERCHANTNUMBER max. length is 10! "${invalidString.length}" given.`);
  });
});