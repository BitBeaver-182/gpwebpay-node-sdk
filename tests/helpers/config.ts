...
import { ConfigParams } from "@/Config/Factory/ConfigFactoryInteface";
...
export const createConfigWithResponseUrl = (defaultGateway = DEFAULT_GATEWAY) => {
  const factory = new ConfigFactory(new PaymentConfigFactory());
  const configWithUrl = Object.entries(configParams)
