...
import { PaymentConfigProvider } from "../src/Config/PaymentConfigProvider";
import { ResponseInterface } from "../src/Data/ResponseInterface";
import { Param } from "../src/Enum/Param";
import { Response } from "../src/Enum/Response";
import { GPWebPayException } from "../src/Exceptions/GPWebPayException";
import { GPWebPayResultException } from "../src/Exceptions/GPWebPayResultException";
import { SignerException } from "../src/Exceptions/SignerException";
import { ResponseFactory } from "../src/Factory/ResponseFactory";
import { ResponseProvider } from "../src/ResponseProvider";
import { SignerFactory } from "../src/Signer/SignerFactory";
import { SignerProvider } from "../src/Signer/SignerProvider";
import { SignerProviderInterface } from "../src/Signer/SignerProviderInterface";
...
describe("ResponseProviderTest", () => {
	const createProvider = () => {
		...
		const signerProvider = new SignerProvider(
			signerFactory,
			config.getSignerConfigProvider(),
		);
	...
	it.each([[Param.DIGEST], [Response.DIGEST1]])(
		"testFailedVerifyResponse - %s",
		(key) => {
			const provider = createProvider();
	...
	it("should call onError when error response is provided", () => {
		...
		expect(errorHandler).toHaveBeenCalledWith(
			expect.any(GPWebPayResultException),
			response,
		);
	});
